            ...
            <tbody>
                <tr v-for="(item, index) in cart" :key="item.product.id">
                    <td class="text-center">
                        ...
                        <button @click="$emit('add-item', item.product)" class="btn btn-info">+</button>
                        <button @click="$emit('delete-item', index)" class="btn btn-outline-info">-</button>
                        </div>
                    </td>
                    <td class="text-center">
                        <img :src="item.product.image" alt="" srcset="" style="width: 100px;">
...
<script>
import Price from "./Price-Items.vue";
export default {
    name: "CheckoutItems",
